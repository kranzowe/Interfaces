...
class ImageListener(Node):
    def __init__(self):
        ...
        self.sub = self.create_subscription(Image,'/camera/camera/color/image_raw',self.img_read, 10)   #sub to frames that are being transmitted, need to f
        self.sub            #prevent unused variable warning
        self.br = CvBridge()    #covert between ROS and OpenCV images

    def img_read(self,msg: Image):
        im_cap = cv2.VideoCapture(0,cv2.CAP_V4L2)
