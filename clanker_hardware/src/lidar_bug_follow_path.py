...
from math import floor, isinf, pi, atan2
import numpy as np
...
class LidarBugNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("steer_p", 100.0)
        ...
        self.pp_lookahead_distance = self.get_parameter("lookahead_distance").value
    ...
    def pose_cb(self, msg: PoseWithCovarianceStamped):
        ...
        self.orientation = self.get_yaw(msg.pose.pose.orientation.w, msg.pose.pose.orientation.x, msg.pose.pose.orientation.y, msg.pose.pose.orientation.z)
        self.pos = np.array([msg.pose.pose.position.x, msg.pose.pose.position.y])

        #find the interections with the path

        currently_inside_lookahead = None
        last_intersection = None

        for start_idx in range(0, len(self.loaded_waypoints.x_pos) - 1):

            segment_start = np.array([self.loaded_waypoints.x_pos[start_idx], self.loaded_waypoints.y_pos[start_idx]])
            segment_end = np.array([self.loaded_waypoints.x_pos[start_idx + 1], self.loaded_waypoints.y_pos[start_idx + 1]])
            dist = np.linalg.norm(segment_end - segment_start)

            # Get the unit vector
            v_unit = (segment_end - segment_start) / dist

            #increments
            increments = floor(dist/ 0.01)
            ...
            for increment in range(0, increments):                    
                ...
                elif(is_in_lookahead == currently_inside_lookahead):
                    last_intersection = incremented_point
                    currently_inside_lookahead = is_in_lookahead
        ...
        if(last_intersection is None):
            ...
            self.get_logger().warn("Outside lookahead")

            #must determine the clostest point
            ...
            for start_idx in range(0, len(self.loaded_waypoints.x_pos) - 1):
                ...
                for increment in range(0, increments):
                    ...
                    if(closest_point is None) or (closest_dist > np.linalg.norm(self.pos - incremented_point)):

                        closest_dist = np.linalg.norm(self.pos - incremented_point)
                        closest_point = incremented_point


            control_point = closest_point

        else:
            
            control_point = last_intersection


        self.get_logger().info(f"{control_point[1]}")


        #determine the current angle that the rover needs to travel at
        delta = control_point - self.pos
        angle_of_travel = atan2(delta[1], delta[0])
    ...
    def load_waypoint_csv(self):
        ...
        self.get_logger().info("Successfully loaded waypoints")
    ...
    def get_yaw(self, w, x, y, z):
        # Standard formula for ZYX convention
        siny_cosp = 2 * (w * z + x * y)
        cosy_cosp = 1 - 2 * (y * y + z * z)
        yaw = atan2(siny_cosp, cosy_cosp)
    
        return yaw # Returns angle in radians
